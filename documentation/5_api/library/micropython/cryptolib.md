# `cryptolib`{.interpreted-text role="mod"} \-- 加密密码

::: {.module synopsis="加密密码"}
cryptolib
:::
...
> - `mode` :
>
>   > - `1` (或 `cryptolib.MODE_ECB` 如果存在）电子代码簿(ECB)
>   > - `2` (cryptolib.MODE_CBC如果存在）密码块链接(CBC)
>   > - `6` (或cryptolib.MODE_CTR如果存在)用于计数器模式(CTR)
>
> - `IV` CBC模式的初始化矢量
>
> - 对于计数器模式，IV是计数器的初始值
