# `_thread`{.interpreted-text role="mod"} \-\-- 线程

该模块提供了用于处理多个线程（也称为轻量级进程或任务）的低级原语 -
多个控制线程共享其全局数据空间。为
...
::: method
[thread.start_new_thread]{#thread.start_new_thread}（function，args
\[，kwargs\]）
:::
...
::: method
[thread.stack_size]{#thread.stack_size}(\[size\])
:::
...
在没有任何可选参数的情况下，此方法无条件地获取锁定，如果有必要，等待它被另一个线程释放（一次只有一个线程可以获取锁定 -
这就是它们存在的原因）。

如果存在整数 `waitflag`
参数，则操作取决于其值：如果它为零，则仅在不等待的情况下立即获取锁定时获取锁定，而如果它非零，则如上所述无条件地获取锁定。

如果浮点超时参数存在且为正，则它指定返回之前的最长等待时间（以秒为单位）。负超时参数指定无限制等待。如果
`waitflag` 为零，则无法指定超时。
