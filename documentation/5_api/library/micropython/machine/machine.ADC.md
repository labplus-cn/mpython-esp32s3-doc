...
# 类 ADC \-- 模数转换 {#machine.ADC}

ADC类提供了模数转换器的接口，表示一个可以采样连续电压并将其转换为离散值的单个端点。

有关ADC采样的额外控制，请参见 [machine.ADCBlock]{.title-ref}。
...
详细引脚定义可查阅
[ESP32引脚功能表.](../../../_images/pinout_wroom_pinout.png)。
:::
...
返回与此ADC对象关联的ADCBlock实例。 此方法仅在端口支持ADCBlock类时存在。
:::
...
设置数据宽度（分辨率）。ADC的分辨率是指它可以将收集的模拟信号转换为数字信号的精度，通常以\"位\"表示，例如，8位意味着ADC可以将指定范围内的电压信号映射到0 -
2\^8-1，即0-255个数字值。更高的分辨率位意味着更精确的表示和更少的信息损失。
