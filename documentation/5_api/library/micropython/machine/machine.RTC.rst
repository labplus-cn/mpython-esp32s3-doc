...
设置RTC闹钟。time可以是毫秒值，用于将闹钟设置为当前时间 + 未来的time_in_ms，也可以是日期时间元组。如果传递的时间是毫秒，则repeat可以设置为True以使闹钟周期性触发。
...
mimxrt端口还将此函数公开为RTC.cancel(alarm_id=0)，但计划在MicroPython 2.0中移除。
