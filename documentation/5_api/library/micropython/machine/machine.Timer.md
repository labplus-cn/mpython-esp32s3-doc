...
请参阅 `micropython.alloc_emergency_exception_buf`{.interpreted-text
role="func"} 如何帮助解决此限制。
::::
...
- `period` - 定时器执行周期（毫秒），每period毫秒执行一次。周期范围：0
  \< period \<= 3435973836
...
::: {.literalinclude caption="定时器控制LED闪烁" linenos=""}
/\_static/examples/timer/timer_led_blink.py
:::
