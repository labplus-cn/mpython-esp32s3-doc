:::: {#servo_api}
::: {.module synopsis="舵机驱动功能函数"}
servo
:::
::::

`servo`{.interpreted-text role="mod"} \-\-- 舵机驱动功能函数 ==========
...
作为Servo控制引脚须为支持PWM(模拟输出)的引脚。掌控板支持PWM的引脚,详情可查阅
...
- 你可以设置 `actuation_range` 来对应用给定的 `min_us` 和 `max_us`
  观察到的实际运动范围值。
