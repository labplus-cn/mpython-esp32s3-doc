:::: {#mpython.py}
::: {.module synopsis="掌控板板载相关功能函数"}
mpython
:::
::::

# `mpython`{.interpreted-text role="mod"} \-\-- 掌控板板载相关功能函数

`mpython` 是基于掌控板封装的专有模块,内含掌控板板载资源相关功能函数。
详细代码实现可查阅 `mpython.py源码 <mpython_code>`{.interpreted-text
role="ref"} 。
...
::: method
numberMap(inputNum,bMin,bMax,cMin,cMax)
:::
...
通过accelerometer对象，您可以获取3轴加速度计值，单位g。
加速度范围±2g/±4g/±8g/±16g,默认为±2g。
...
注意,校准数据断电后不会保存。`x` , `y` , `z`
为调整偏差值,可修正范围±1g。 掌控板v2.3版本以上，掉电会保存校准数据。
...
::: {.literalinclude caption="accelerometer 事件的简单应用" linenos=""}
/\_static/examples/accelerometer/accelerometer_event.py
:::

# 陀螺仪(gyroscope)

通过gyroscope对象，您可以获取陀螺仪角速度值，角速度的单位是dps(°/S)。
角速度范围±16dps/±32dps/±64dps/±128dps/±256dps/±512dps/±1024dps/±2048dps
,默认为±256 dps。
...
磁力去皮。类似电子秤去皮功能, `peeling()` 后,下次 `get_field_strength()`
...
::: {.literalinclude caption="磁力计应用--指北针" linenos=""}
/\_static/examples/magnetic/compass.py
:::

# 蜂鸣器(music)

由 `music` 模块驱动掌控板蜂鸣器,具体操作详见 `music`{.interpreted-text
role="mod"} 模块。

# [button]()\[a,b\]对象

掌控板上的a,b按键。button_a/button_b 是 `Button` 类的实例对象。使用
`machine.Pin.irq<Pin.irq>`{.interpreted-text role="ref"}
中断实现。定义了 `event_pressed` 和 `event_released`
按键按下、释放事件。
用户可轻易的实现事件回调。除此外，还实现当前或过去按键状态、按键次数等函数方法。

::: {.Button(pin_num, .reverse=False)}
Button类，按键抽象类。
:::
...
当按键事件发生，触发事件回调。回调函数定义如，function_callback(pin),
`pin` 为该引脚的machine.Pin对象返回。
...
::: {.literalinclude caption="Button 事件回调的简单应用" linenos=""}
/\_static/examples/button/button_event.py
:::
...
返回 `True` 或 `False`
指示自设备启动以来或上次调用此方法以来是否按下按钮。调用此方法将清除按下状态，因此必须再次按下按钮，然后才能再次返回
`True` 。
...
:::: {#button.irq}
::: method
Button.irq(handler=None, trigger=(Pin.IRQ_FALLING \| Pin.IRQ_RISING),
priority=1, wake=None)
:::
::::

配置在引脚的触发源处于活动状态时调用的中断处理程序。用法与
machine.Pin.irq 一样。
...
>
> - `wake` 选择此中断可唤醒系统的电源模式。它可以是 `machine.IDLE` ，
>   `machine.SLEEP` 或 `machine.DEEPSLEEP` 。
>
...
触摸按键按下事件。当按键事件发生，触发事件回调。回调函数定义如，function_callback(value),
`value` 为该触摸按键的状态值。
...
用于控制掌控板的3颗RGB
ws2812灯珠。rgb对象为neopixel的衍生类，继承neopixel的方法。更多的使用方法请查阅
`neopixel<neopixel>`{.interpreted-text role="ref"} 。
...
mPython掌控板已实例 `I2C` 类，P19、P20
为I2C的SCL、SDA引脚。I2C设备可连接掌控板I2C总线进行操作。

详细有关I2C的读写操作，请查看
`machine.I2C<machine.I2C>`{.interpreted-text role="ref"} 模块或
`I2C基础教程<tutorials_i2c>`{.interpreted-text role="ref"} 章节。

# MPythonPin类

::: {.MPythonPin(pin, .mode=PinMode.IN,pull=None)}
构建Pin对象
:::

- `pin` 掌控板定义引脚号，具体定义看查看
  `掌控板引脚定义<mpython_pinout>`{.interpreted-text role="ref"} 。

- `mode` 引脚模式。未设定时,默认 [mode]{.title-ref} =
  [PinMode.IN]{.title-ref}
