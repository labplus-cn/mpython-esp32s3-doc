:::: {#gui}
::: {.module synopsis="提供GUI类的绘制的相关功能函数"}
gui
:::
::::

`gui`{.interpreted-text role="mod"} \-\-- 提供GUI类的绘制的相关功能函数
==========
...
支持 [pbm]{.title-ref} 和 [bmp]{.title-ref} 1bit的图片格式。
...
加载 [pbm]{.title-ref} 或 [bmp]{.title-ref} 图片格式文件,返回该图片的
`framebuf.FrameBuffer`{.interpreted-text role="class"} 对象。
