::: {.module synopsis="提供无线蓝牙功能"}
bluetooth
:::

# `bluetooth`{.interpreted-text role="mod"} \-\-- 提供无线蓝牙功能

该模块提供ble蓝牙通讯、HID模拟设备功能。

## ble类

ble类, 支持客户端与主机端的BLE通讯及HID功能。HID功能,遵循BLE
HID规范,可实现模拟无线蓝牙HID设备(如鼠标，键盘，游戏手柄等人机交互设备)。

### BLE 通讯 {#ble-通讯 .ble}

> BLE客户端设备初始化。当不带参数时,默认客户端设备名为
> [mpython]{.title-ref} 。
>
> > - `name` - 客户端设备名称,字符串类型。
...
> 接收BLE主机端的数据的回调函数, [f]{.title-ref}
> 为回调函数,函数定义如下, `f(bytearray)` 。

> BLE客户端设备关闭广播

> 模拟HID消费类设备的控制器。如,蓝牙遥控器。
>
> - `第一参数` - HID Consumer类的常用的按键ID。如你想控制电视机开机,则用
>   [ble.HID_CONSUMER_POWER]{.title-ref} 。具体的按键常量详见
>   `HID Consumer ID表<HID Consumer Usage IDs>`{.interpreted-text
>   role="ref"} 。
> - `第二参数`- 按键状态, [True]{.title-ref} 为按下, [False]{.title-ref}
>   为松开。
>
> 电视遥控音量加减的示例:
>
...
>
> - `keys` -
>   一个或多个按键的数组类型。当数组内成员表示为按下的按键,如要释放按键只需将数组内对应位置的值赋值为0。具体的按键常量详见
>   `HID Keyboard ID表<HID Keyboard Usage IDs>`{.interpreted-text
>   role="ref"} 。
>
> - `modifier` - 键盘组合按键,默认为 [KEY_MASK_NONE]{.title-ref}
>   表示为单按键 ,可以为以下值。如要用到2个组合按键,可使用 `|`
>   或逻辑运算,像 [Ctrl]{.title-ref} + [Alt]{.title-ref} 组合按键时为
>   [KEY_MASK_L_CTRL \| KEY_MASK_L_ALT]{.title-ref}
>
>   > - [KEY_MASK_NONE]{.title-ref} - 0
>   > - [KEY_MASK_L_CTRL]{.title-ref} - 1
>   > - [KEY_MASK_L_SHIFT]{.title-ref} - 2
>   > - [KEY_MASK_L_ALT]{.title-ref} - 4
>   > - [KEY_MASK_L_GUI]{.title-ref} - 8
>   > - [KEY_MASK_R_CTRL]{.title-ref} - 16
>   > - [KEY_MASK_R_SHIFT]{.title-ref} - 32
>   > - [KEY_MASK_R_ALT]{.title-ref} - 64
>   > - [KEY_MASK_R_GUI]{.title-ref} - 128
>
> 键盘示例:
>
>     # Ctrl + F组合按键
...
::: {.literalinclude caption="PPT 翻页遥控器" linenos=""}
/\_static/examples/bluetooth/hid_ppt_Remote_Control.py
:::
