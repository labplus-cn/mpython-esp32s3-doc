# `lcd160cr`{.interpreted-text role="mod"} \-\-- control of LCD160CR display

::: {.module synopsis="control of LCD160CR display"}
lcd160cr
:::

This module provides control of the MicroPython LCD160CR display.

![LCD160CRv1.0 picture](http://micropython.org/resources/LCD160CRv10-persp.jpg){width="640px"}

Further resources are available via the following links:

- [LCD160CRv1.0 reference
  manual](http://micropython.org/resources/LCD160CRv10-refmanual.pdf)
  (100KiB PDF)
- [LCD160CRv1.0
  schematics](http://micropython.org/resources/LCD160CRv10-schematics.pdf)
  (1.6MiB PDF)
...
::: {.LCD160CR(connect=None, .\\*, .pwr=None, .i2c=None, .spi=None, .i2c_addr=98)}
...
> - \"X\" is for the X-skin and uses: `pwr=Pin("X4")`, `i2c=I2C("X")`,
>   `spi=SPI("X")`
> - \"Y\" is for the Y-skin and uses: `pwr=Pin("Y4")`, `i2c=I2C("Y")`,
>   `spi=SPI("Y")`
...
See [this
image](http://micropython.org/resources/LCD160CRv10-positions.jpg) for
how the display can be connected to the pyboard.
:::
...
`LCD160CR.set_text_color`{.interpreted-text role="meth"}) pen color (see
`LCD160CR.set_pen`{.interpreted-text role="meth"}) and draw individual
pixels.
:::
...
members are updated when calling `LCD160CR.set_orient`{.interpreted-text
role="meth"} and should be considered read-only.
:::
...
[PORTRAIT]{.title-ref}, [LANDSCAPE]{.title-ref},
[PORTRAIT_UPSIDEDOWN]{.title-ref}, [LANDSCAPE_UPSIDEDOWN]{.title-ref}.
:::
...
a logical or of [STARTUP_DECO_NONE]{.title-ref},
[STARTUP_DECO_MLOGO]{.title-ref}, [STARTUP_DECO_INFO]{.title-ref}.
:::
...
integer and can be created by `LCD160CR.rgb`{.interpreted-text
role="meth"}.
:::
...
values, set by the size of the screen minus the given *x* and *y*
values. *buf* should be large enough to hold `2*w*h` bytes. If it\'s
smaller then only the initial horizontal lines will be stored.
:::
...
[write]{.title-ref} to draw the text.

::: method
LCD160CR.set_pos(x, y)

Set the position for text output using
`LCD160CR.write`{.interpreted-text role="meth"}. The position is the
...
Set the font for the text. Subsequent calls to [write]{.title-ref} will
use the newly configured font. The parameters are:

> - *font* is the font family to use, valid values are 0, 1, 2, 3.
> - *scale* is a scaling value for each character pixel, where the
>   pixels are drawn as a square with side length equal to *scale + 1*.
>   The value can be between 0 and 63.
> - *bold* controls the number of pixels to overdraw each character
>   pixel, making a bold effect. The lower 2 bits of *bold* are the
>   number of pixels to overdraw in the horizontal direction, and the
>   next 2 bits are for the vertical direction. For example, a *bold*
>   value of 5 will overdraw 1 pixel in both the horizontal and vertical
>   directions.
...
display supports basic VT100 control codes such as newline and
backspace.
:::
...
for the outline, and the pen fill color for the interior. The
[rect]{.title-ref} method draws the outline and interior, while the
other methods just draw one or the other.
:::
...
Similar to `LCD160CR.poly_dot`{.interpreted-text role="meth"} but draws
...
> - If *calib* is `True` then the call will trigger a touch calibration
>   of the resistive touch sensor. This requires the user to touch
...
> - If *irq* is `True` then the display will be configured to pull the
>   IRQ line low when a touch force is detected. If *irq* is `False`
>   then this feature is disabled. If *irq* is `None` (the default
>   value) then no change is made to this setting.
:::
...
screen, [False]{.title-ref} otherwise.
:::
...
location of the first byte to go to the top-left corner of the window
set by `LCD160CR.set_spi_win`{.interpreted-text role="meth"}. The method
returns an SPI object which can be used to write the pixel data.
...
of the window specified by `LCD160CR.set_spi_win`{.interpreted-text
role="meth"} it will wrap around to the top-left corner of that window.
:::
...
containing the 16-bit RGB values for the pixels, and they will be
written to the area specified by
`LCD160CR.set_spi_win`{.interpreted-text role="meth"}, starting from the
top-left corner.

The [framebuf](framebuf.html) module can be used to construct frame
...
::: method
LCD160CR.set_scroll_win(win, x=-1, y=0, w=0, h=0, vec=0, pat=0,
fill=0x07e0, color=0)

Configure a window region for scrolling:

> - *win* is the window id to configure. There are 0..7 standard windows
>   for general purpose use. Window 8 is the text scroll window (the
>   ticker).
> - *x*, *y*, *w*, *h* specify the location of the window in the
>   display.
> - *vec* specifies the direction and speed of scroll: it is a 16-bit
>   value of the form `0bF.ddSSSSSSSSSSSS`. *dd* is 0, 1, 2, 3 for +x,
>   +y, -x, -y scrolling. *F* sets the speed format, with 0 meaning that
>   the window is shifted *S % 256* pixel every frame, and 1 meaning
>   that the window is shifted 1 pixel every *S* frames.
> - *pat* is a 16-bit pattern mask for the background.
> - *fill* is the fill color.
> - *color* is the extra color, either of the text or pattern
>   foreground.
:::
...
>   to the parameters in the [set_scroll_win]{.title-ref} method.
...
`LCD160CR.set_pos`{.interpreted-text role="meth"}.
:::
...
be a single call to [jpeg_start]{.title-ref} to begin with, specifying
the total number of bytes in the JPEG. Then this number of bytes must be
transferred to the display using one or more calls to the
[jpeg_data]{.title-ref} command.
:::

::: method
LCD160CR.feed_wdt()

The first call to this method will start the display\'s internal
watchdog timer. Subsequent calls will feed the watchdog. The timeout is
roughly 30 seconds.
:::
...
Orientations of the display, used by
`LCD160CR.set_orient`{.interpreted-text role="meth"}.
:::
...
`LCD160CR.set_startup_deco`{.interpreted-text role="meth"}.
:::
