# `collections`{.interpreted-text role="mod"} \-- 容器数据类型
...
> deque.appendleft(x)
>
> 在deque的左边加上x。如果启用溢出检查，并且没有剩余的空间，则会引发索引错误。
> :::
>
> ::: method
> deque.pop()
>
> 从deque的右侧移除并返回一个项目。如果没有项出现，就会引起索引错误。
> :::
>
> ::: method
> deque.popleft()
...
>
> 通过将iterable中的所有项目追加到deque的右侧来扩展deque。如果启用溢出检查，并且没有剩余的空间，则会引发索引错误。
> :::
:::
...
Output:
    ...
    从字典中删除并返回一个(key, value)对。对以LIFO（后进先出）顺序返回。

    .. admonition:: 与CPython的区别
       :class: attention

       OrderedDict.popitem()不支持last=False参数，并且如果存在，将始终删除并返回最后一项。
       解决方法是使用pop(<first_key>)来删除第一项：
