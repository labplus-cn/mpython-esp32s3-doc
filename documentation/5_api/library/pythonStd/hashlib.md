# `hashlib`{.interpreted-text role="mod"} \-- 散列算法
...
::: hashlib.sha256([data])
创建一个SHA256哈希对象，并可选择将 `data` 输入其中。
:::

::: hashlib.sha1([data])
创建一个SHA1哈希对象，并可选择将 `data` 输入其中。
:::

::: hashlib.md5([data])
创建一个MD5哈希对象，并可选择将 `data` 输入其中。
:::
...
此方法未实现。使用 `binascii.hexlify(hash.digest())` 来达到类似的效果。
:::
