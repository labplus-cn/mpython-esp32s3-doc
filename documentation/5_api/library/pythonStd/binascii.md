# `binascii`{.interpreted-text role="mod"} \-- 二进制/ ASCII互转

::: {.module synopsis="二进制/ ASCII互转"}
binascii
:::
...
3548](https://tools.ietf.org/html/rfc3548.html) 中所述。如果 newline 为
True，则返回编码数据，后跟换行符，作为 `bytes` 对象。
:::

::: function
crc32(data\[, value\])

计算数据的32位校验和CRC-32，从初始CRC值开始。默认初始CRC为零。该算法与ZIP文件校验和一致。
:::
