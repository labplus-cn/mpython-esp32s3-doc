# `os`{.interpreted-text role="mod"} \-- 基本的操作系统
...
该uos模块包含文件系统访问和安装，终端重定向和复制 `uname` 以及 `urandom`
功能的功能。
...
复制或切换给定类似 `stream` 对象上的MicroPython终端（REPL）。所述
[stream_object]{.title-ref} 参数必须是一个本地流对象，或从导出
`uio.IOBase` 并实施 `readinto()` 和 `write()`
...
如果 `None` 作为 [stream_object]{.title-ref}
传递，则在索引给出的槽上取消复制。
...
这允许Python程序看到的文件系统的动态和灵活配置。提供 `mount()` 和
`umount()` 功能，以及可能由VFS类表示的各种文件系统实现。

::: function
mount(fsobj, mount_point, \*, readonly)

将文件系统对象 [fsobj]{.title-ref}
挂载到mount_point字符串指定的VFS中的位置。 fsobj可以是具有 `mount()`
方法或块设备的VFS对象。如果它是块设备，则会自动检测文件系统类型（如果未识别文件系统，则会引发异常）。
[mount_point]{.title-ref} 可以是在根目录下\'/\'挂载 [fsobj]{.title-ref}
，也\'/\<name\>\'可以将它挂载在根目录下的子目录中。
:::

如果是 [readonly]{.title-ref} 为 [True]{.title-ref}
则文件系统以只读方式挂载。

在挂载过程中，将 `mount()` 在文件系统对象上调用该方法。

`OSError(EPERM)` 如果 [mount_point]{.title-ref} 已经安装，则会引发。

::: function
umount(mount_point)

卸载文件系统。\`mount_point\`
可以是命名安装位置的字符串，也可以是先前安装的文件系统对象。在卸载过程中，将
[umount()]{.title-ref} 在文件系统对象上调用该方法。
:::

`OSError(EINVAL)` 如果找不到 [mount_point]{.title-ref} ，则会引发。

:::: VfsFat(block_dev)
创建使用 [FAT]{.title-ref}
文件系统格式的文件系统对象。FAT文件系统的存储由 [block_dev]{.title-ref}
提供。可以使用此构造函数创建的对象 `mount()` 。

::: staticmethod
mkfs(block_dev)

在 [block_dev]{.title-ref} 上构建FAT文件系统。
:::
::::
