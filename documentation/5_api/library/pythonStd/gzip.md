# `gzip`{.interpreted-text role="mod"} \-\-- gzip压缩和解压缩

::: {.module synopsis="gzip压缩和解压缩"}
gzip
:::

该模块实现了相应 `CPython`{.interpreted-text role="term"}
模块的一个子集，如下所述。有关更多信息，请参阅原始CPython文档:
[gzip](https://docs.python.org/3.5/library/gzip.html#module-gzip)
...
建议使用 deflate.DeflateIO
而不是此模块中的函数，因为它提供了压缩和解压缩的流式接口，在处理从文件、套接字或流读取或写入压缩数据时更方便且内存效率更高。
::::
