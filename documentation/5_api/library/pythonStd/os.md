...
该os模块包含文件系统访问和安装，终端重定向和复制 `uname` 以及 `urandom`
功能的功能。
...
> - *size*
>   是一个整数，可能根据文件系统类型包含。对于文件条目，size表示文件的大小或-1（如果未知）。对于目录条目，其含义目前未定义。
...
`io.IOBase` 并实施 `readinto()` 和 `write()`
...
此函数应由自定义流实现（例如UART、Bluetooth或其他非USB
REPL流）调用，以通知REPL输入已准备好读取。正确使用可确保REPL及时处理特殊字符（如用于触发KeyboardInterrupt的Ctrl+C），从而为用户代码启用预期的中断行为。
...
以下函数和类已移至vfs模块。它们仅在此模块中提供以实现向后兼容性，并将在MicroPython
2版本中删除。
