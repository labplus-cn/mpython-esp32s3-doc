# `io`{.interpreted-text role="mod"} \-- 输入/输出流
...
在某些地方，这种组合是禁止的，在某些情况下它是未定义的或只是没有记录，在某些情况下它会引发冗长的异常。这个问题在MicroPython中要简单得多：非阻塞流对于高效的异步操作很重要，因此这个属性在\"no-short-ops\"上占优势。
所以，虽然阻塞流会尽可能避免短读/写（获得短读的唯一情况是达到文件末尾，或在错误的情况下（但错误不会返回短数据，而是引发异常）），但非阻塞流可能会产生短数据以避免阻塞操作。
...
所有提供文件系统访问的端口都需要支持mode参数，但对其他参数的支持因端口而异。
:::
