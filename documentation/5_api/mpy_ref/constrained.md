...
最好避免导入时运行的代码；
更好的方法是在所有模块被导入后都有由应用程序运行的初始化代码。
这一方法将编译器可用的RAM最大化。
...
从而也节省了RAM。但是 `ROWS`
值将占用至少两个机器字，两个字分别对应globals字典中的键值和值。
必须出现在字典中，因为另一个模块可能会导入或使用它。这个RAM可通过在名称前加下划线前面（如
`_COLS` ）来保存。 这个
RAM可以通过用下划线作为in_COLS的名字来保存：这个符号在模块外不可见，所以不会占用RAM。

`const()` 的参数可为在编译时计算结果为整数的任何值，如 `0x100` 或
`1 << 8` 。 甚至可包括其他已定义的常量符号，如 `1 << BIT` 。

**常量数据结构**

若存在大量常量数据，且平台支持从Flash执行，则RAM可能会如下保存。数据应该位于Python模块中并冻结为字节码。
数据必须定义为 [bytes]{.title-ref} 对象。编译器\"知道\"
[bytes]{.title-ref}
对象是不可变的，并确保对象保留在闪存中，而不是被复制到RAM中。
[ustruct]{.title-ref} 模块可协助类型和其他Python内置类型间的转换。
...
实际的字符串\"The quick brown
fox\"将停留在Flash中。运行时，字符串的引用被分配给变量 `mystring` 。
...
创建动态字符串的最佳方式是通过字符串 [format]{.title-ref} 方法:
...
首次调用中，在RAM中创建一个整数元组。第二次调用有效地创建消耗最小RAM的
`bytes` 对象。 若模块被冻结为字节码，则 `bytes` 对象将保留在Flash中。

**字符串vs字节**

Python3引入了Unicode支持，也就引入了字符串和字节数组之间的区别。只要字符串中的所有字符都为ASCII（即值\<126），
MicroPython即可确保Unicode字符串不占用额外空间。若需完整8位范围内的值，则可使用
[bytes]{.title-ref} 和 [bytearray]{.title-ref} 对象来确保无需额外空间。
请注意：大多数字符串方法（例如 `str.strip()`{.interpreted-text
role="meth"}）也适用于 [bytes]{.title-ref}
实例，所以消除Unicode并不困难。
...
在需在字符串和字节之间进行转换之处，可使用 [str.encode()]{.title-ref} 和
[bytes.decode()]{.title-ref} 方法。请注意：字符串和字节都是不可变的。
...
Python的函数 [eval]{.title-ref} 和 [exec]{.title-ref}
在运行时调用编译器，这需要大量的RAM。请注意：来自
[micropython-lib]{.title-ref} 的 [pickle]{.title-ref} 库使用
[exec]{.title-ref} 。使用 [ujson]{.title-ref}
库进行对象序列化可能会更高效地利用RAM。
...
然后将所有Q(xxx)行复制并粘贴到文本编辑器中。检查并删除明显无效的行。
打开将在stmhal中（或使用中的架构的等效目录）
...
这个过程自动进行，但可通过发出 [gc.collect()]{.title-ref} 来直接调用。
...
程序创建对象 `foo` ，然后创建对象 `bar` 。随后 `foo` 超出范围，但 `bar`
仍保留。 `foo` 所占用的 RAM将被GC回收。但是，若 `bar`
被分配到更高地址，从 `foo` 回收的RAM只能用于不大于 `foo` 的对象。
在复杂或长时间运行的程序中，堆可进行碎片化处理：尽管存在大量可用的RAM，但并无足够的连续空间来分配特定对象，且程序因存储器错误而失效。

上述技术旨在最大限度地减少这种情况。
在需要大的永久性缓冲区或其他对象的情况下，最好在程序执行过程中、
碎片化进行前尽早将这些缓冲区实例化。
可通过监视堆的状态和控制GC来进一步改进。概述如下。

### 报告

许多库函数可用于报告内存分配和控制GC。这些都可以在 [gc]{.title-ref} 和
[micropython]{.title-ref} 模块中找到。 下面的例子可能被粘贴在REPL（ctrl
e进入粘贴模式，ctrl d运行它）。许多库函数可用于报告内存分配并控制GC。
这些同样存在 [gc]{.title-ref} 和 [micropython]{.title-ref}
模块中。以下示例可能粘贴到REPL中（ `ctrl e` 进入粘贴模式， `ctrl d`
运行它）。
...
- [gc.collect()]{.title-ref} 强制执行垃圾收集。见脚注。
- [micropython.mem_info()]{.title-ref} 打印RAM利用率的总结。
- [gc.mem_free()]{.title-ref} 返回空闲堆大小（以字节为单位）。
- [gc.mem_alloc()]{.title-ref} 返回当前分配的字节数量。
- `micropython.mem_info(1)` 打印堆利用率的表格（详情见下）。

生成的数字取决于平台，但可以看到，定义函数使用由编译器发出的字节码形式的少量RAM（编译器使用的RAM已被回收）。
运行该函数使用超过10KiB，但返回时， `a`
为垃圾，因为它超出范围且无法引用。最后的 [gc.collect()]{.title-ref}
会恢复内存。
...
可随时通过发出 [gc.collect()]{.title-ref}
来请求GC。定期执行首先有助于防止碎片化，其次也有利于提高性能。
...
则在导入后续模块时，编译器可能会缺乏可用RAM。若模块在导入时实例化数据，那么在导入后发出的
[gc.collect()]{.title-ref} 会改善这一问题。
...
驻留字符串被称为 `qstr`
。在正常导入的模块中，单个实例将位于RAM中，但如上所述，在冻结为字节码的模块中，则将位于Flash中。
...
见 [array]{.title-ref} 、 [ustruct]{.title-ref} 和 [uctypes]{.title-ref}
模块。

### 脚注：gc.collect()返回值

在Unix和Windows平台上， [gc.collect()]{.title-ref}
方法返回一个整数，该整数表示在回收中收回的不同内存
区域的数量（更确切地说，是变为空闲块的head
block的数量）。出于效率原因，baremetal端口不返回此值。
