...
符号：除使用R0-R15的特殊指令的情况外， `Rd, Rn` 表示ARM寄存器R0-R7。
`Rn<a-b>` 表示内容介于 `a <= contents <= b` 范围的ARM寄存器。
对于有两个寄存器参数的指令，允许二者相同。例如，无论初始内容如何，以下指令都将把R0归零（Python
`R0 ^= R0` ）。
...
三位旋转运行如下。若Rd初始就包含位 `b31 b30..b0` ，则旋转后将包含
`b2 b1 b0 b31 b30..b3` 。
