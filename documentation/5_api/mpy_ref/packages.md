...
2.  [upip]{.title-ref} package manager can be used to install a
    distribution package on a [MicroPython port]{.title-ref} with
    networking capabilities (for example, on the Unix port).
...
[upip]{.title-ref} package manager doesn\'t execute `setup.py` during
...
packages not compatible with [CPython]{.title-ref}\'s package manager,
`pip`. This isn\'t considered a big problem, because:

1.  Packages can be installed with [upip]{.title-ref}, and then can be
    used with CPython (if they are compatible with it).
...
[upip]{.title-ref} package manager. [upip]{.title-ref} is a Python
application which is usually distributed (as frozen bytecode) with
network-enabled [MicroPython ports \<MicroPython port\>]{.title-ref}. At
the very least, [upip]{.title-ref} is available in the [MicroPython Unix
port]{.title-ref}.

On any [MicroPython port]{.title-ref} providing [upip]{.title-ref}, it
...
don\'t. This is because PyPI provides a central package repository for
all different Python implementations and versions, and thus distribution
package names may need to be namespaced for a particular implementation.
For example, all packages from [micropython-lib]{.title-ref} follow this
naming convention: for a Python module or package named `foo`, the
distribution package name is `micropython-foo`.

For the ports which run MicroPython executable from the OS command
prompts (like the Unix port), [upip]{.title-ref} can be (and indeed,
usually is) run from the command line instead of MicroPython\'s own
REPL. The commands which corresponds to the example above are:
...
For [MicroPython ports \<MicroPython port\>]{.title-ref} without native
networking capabilities, the recommend process is \"cross-installing\"
them into a \"directory image\" using the [MicroPython Unix
port]{.title-ref}, and then transferring this image to a device by
suitable means.
...
`install_dir/` prefix) to the device, at the suitable location, where it
can be found by the Python `import` statement (see discussion of the
[upip]{.title-ref} installation path above).

# Cross-installing packages with freezing

For the low-memory [MicroPython ports \<MicroPython port\>]{.title-ref},
the process described in the previous section does not provide the most
efficient resource usage,because the packages are installed in the
source form, so need to be compiled to the bytecome on each import. This
...
given [MicroPython port]{.title-ref} from the C source code.
...
1.  Follow the instructions for a particular port on setting up a
    ...
    executable/firmware successfully before proceeding to the next
    steps.
2.  Build [MicroPython Unix port]{.title-ref} and make sure it is in
    your PATH and you can execute `micropython`.
3.  Change to port\'s directory (e.g. `ports/esp8266/` for ESP8266).
4.  Run `make clean-frozen`. This step cleans up any previous modules
    which were installed for freezing (consequently, you need to skip
    this step to add additional modules, instead of starting from
    scratch).
...
1.  Step 5 in the sequence above assumes that the distribution package
    is available from PyPI. If that is not the case, you would need to
    copy Python source files manually to `modules/` subdirectory of the
    port port directory. (Note that upip does not support installing
    from e.g. version control repositories).
2.  The firmware for baremetal devices usually has size restrictions, so
    adding too many frozen modules may overflow it. Usually, you would
    get a linking error if this happens. However, in some cases, an
    image may be produced, which is not runnable on a device. Such cases
    are in general bugs, and should be reported and further
    investigated. If you face such a situation, as an initial step, you
    may want to decrease the amount of frozen modules included.

# Creating distribution packages

Distribution packages for MicroPython are created in the same manner as
for CPython or any other Python implementation, see references at the
end of chapter. Setuptools (instead of distutils) should be used,
because distutils do not support dependencies and other features.
\"Source distribution\" (`sdist`) format is used for packaging. The
post-processing discussed above, (and pre-processing discussed in the
...
A complete application, besides the source code, oftentimes also
consists of data files, e.g. web page templates, game images, etc. It\'s
clear how to deal with those when application is installed manually -
you just put those data files in the filesystem at some location and use
the normal file access functions.
...
more advanced approach to accessing data files. This approach is
treating the data files as \"resources\", and abstracting away access to
them.
...
`pkg_resources.resource_stream(package, resource)` function. The idea is
that an application calls this function, passing a resource identifier,
which is a relative path to data file within the specified package
(usually top-level application package). It returns a stream object
which can be used to access resource contents. Thus, the
`resource_stream()` emulates interface of the standard
[open()]{.title-ref} function.
...
extra intermediate step when packaging application - creation of
...
`__main__.py` and `utils.py` should access resources using the following
calls:
...
You can develop and debug using the [MicroPython Unix port]{.title-ref}
as usual. When time comes to make a distribution package out of it, just
use overridden \"sdist\" command from sdist_upip.py module as described
in the previous section.

This will create a Python resource module named `R.py`, based on the
files declared in `MANIFEST` or `MANIFEST.in` files (any non-`.py` file
will be considered a resource and added to `R.py`) - before proceeding
with the normal packaging steps.
...
MicroPython distribution, in which can you will need to pass paths to
all resource files:
