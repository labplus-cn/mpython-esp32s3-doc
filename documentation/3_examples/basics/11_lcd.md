...
掌控板2.0 板载1.47英寸lcd显示屏，分辨率320x172。内置 `LVGL` 的
`lv_micropython` 库，因此可以使用lvgl的相关API来进行屏幕绘制。
...
[LVGL(轻量级和通用图形库)](https://docs.lvgl.io/master)
是一个免费和开源的图形库，它提供了创建嵌入式GUI所需的一切，具有易于使用的图形元素，美丽的视觉效果和低内存占用。
::::
...
软件中使用的 `GUI` 库为根据 `lvgl`
自主封装的库。以下所讲解的都是通过GUI库来绘制屏幕，若想使用原生
`lv_micropython` 进行编程可自行前往
[LVGL官网](https://docs.lvgl.io/master) 或 [lv_micropython
github仓库](https://github.com/lvgl/lv_micropython) 进行学习。
::::
...
![image](/_static/image/example/lcd/draw_label.png" width="400"
alt="显示效果" />
<figcaption aria-hidden="true">显示效果</figcaption>


## 屏幕的坐标点

掌控板屏幕分辨率为 **320 x 172**，左上角坐标为 **(0, 0)**，右下角坐标为
**(320, 172)**。横向向右x坐标增大，纵坐标向下y坐标增大。


![image](/_static/image/example/lcd/lcd_size.png" width="400" />
...
![image](/_static/image/example/lcd/lcd_color.png" width="400"
alt="效果图" />
...
参数 `color` 颜色的色值为16进制数值，即 **0x000000 \~
0xFFFFFF**，也可以写成 **0 \~ 16777215** 。
::::
...
| gui.draw_line(x, y, width, height, border_width, border_color,
  bg_color, radius, fill, draw)
| `x`、 `y` 为左上角坐标(x, y)
| `width` 为宽度
| `height` 为高度
| `border_width` 为边框粗细
...
| gui.draw_round(x, y, radius, border_width, border_color, bg_color,
  fill, draw)
| `x`、 `y` 为圆心坐标(x, y)
| `radius` 为半径
| `border_width` 为边框粗细
...
接下来是使用取模工具对图片进行取模。网上有PCtoLCD、lcd image
converter等取模软件，可根据自己喜好自行选择。

- 步骤1.导入320x172，bmp格式图片
- 步骤2.选择参数，输出数据类型\[C语言数组\]、
  扫描模式\[水平扫描\]、输出灰度\[单色\]、宽高\[320\*172\]
- 步骤3.点击保存，自动生成取模数据。
...
![image](/_static/image/example/lcd/lcd_bmp.png" alt="效果图" />
<figcaption aria-hidden="true">效果图</figcaption>
...
![image](/_static/image/example/lcd/lcd_upload_img.png" width="400"
alt="操作流程图" />
<figcaption aria-hidden="true">操作流程图</figcaption>
...
![image](/_static/image/example/lcd/lcd_img.png" width="400"
alt="效果图" />
<figcaption aria-hidden="true">效果图</figcaption>
