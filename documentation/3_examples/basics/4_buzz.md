...
掌控板板载喇叭，通过主控IO产生不同频率的脉冲信号，经功率放大后，通过喇叭发出声音。频率不同，发出的音调就不一样，从而可以发出不同的声音，可以做出"多来米发索拉西"的效果。
...
每个音符都有一个名字（比如C＃或F），一个八度和一个持续时间。八度用数字表示〜0表示最低八度，4表示中央C，8表示您需要的高度。持续时间也表示为数字。
持续时间的值越高，持续时间越长。例如，持续时间4将持续两倍于持续时间2（依此类推）。
每个音符都表示为一串字符，如下所示： :
