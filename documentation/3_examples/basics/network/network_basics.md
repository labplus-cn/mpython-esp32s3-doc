...
::: {#network_base}
MicroPython `network`{.interpreted-text role="mod"}
模块用于配置WiFi连接。有两个WiFi接口，STA模式即工作站模式（ESP32连接到路由器），
AP模式提供接入服务（其他设备连接到ESP32）。如需了解MicroPython的网络连接方法，请查阅
`network`{.interpreted-text role="mod"} 模块。
:::

## STA模式

掌控板以基于network模块封装
`mpython.wifi()<mpython.wifi>`{.interpreted-text role="ref"}
类简化wifi连接设置:
...
实例化wifi()后，会构建 `sta` 和 `ap` 两个对象。 `sta`
对象为工作站模式，通过路由器连接至网络。`ap` 为AP模式，提供wifi接入。
::::
...
`wifi.enable_APWiFi(essid,password)` 用于配置并开启AP模式函数, `essid`
参数为wifi名称, `password`
