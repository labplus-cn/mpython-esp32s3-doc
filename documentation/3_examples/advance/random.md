...
有时我们需要做些随机行为或生产随机数。这时你可以使用
`random<random>`{.interpreted-text role="ref"} 模块。
...
每次重启掌控板时，它都会显示一个介于1和6之间的数字。`randint()`
返回的是整形，我们需要使用 `str()` 将整形转为字符串(例如，6 -\> \"6\")。
...
random.randrange(start, end, step)。`start` 为随机数开始值，`end`
为随机数结束值，step为递增基数。 以上例子是随机显示(0,10)范围的偶数。
::::

有时您需要带小数点的数字。你可以使用 `random.random`
方法生成0.0到1.0的随机浮点数。如果你需要较大的随机浮点数加的结果
`random.uniform` :
