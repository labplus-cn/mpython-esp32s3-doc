...
精度为±3％，气压为±1
hPa绝对精度，温度精度为±1.0°C。由于压力随高度变化，压力测量结果非常好，您也可以将其用作±1米或更高精度的高度计！
...
mpython内已实例化 `BME280` 类,直接使用 `bme280`
对象即可。获取温度值,使用 `temperature()`
函数,单位摄氏度。获取湿度值,使用 `humidity()` ,单位%。获取大气压强,使用
`pressure()` ,单位Pa(帕)。

![image](/_static/image/classic/bme280.png){.align-center}

## 拓展应用

现在你已学会了如何获取BME280的气象数据了。进一步思考,如何利用气象数据和如何将它送至远端呢？你可结合之前学习的http
Server供远程访问或者使用mqtt协议将数据推送送至服务器。
