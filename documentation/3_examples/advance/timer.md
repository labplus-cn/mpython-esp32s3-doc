...
::: {.literalinclude caption="定时警报器" linenos=""}
/\_static/examples/timer/timer_alarm.py
:::
...
`Timer.init(period=5000, mode=Timer.PERIODIC,callback=None)`
用来初始化计时器，\`mode\` 可以是以下之一：\`Timer.ONE_SHOT\`
指计时器运行一次，直到配置完毕通道的期限到期；\`Timer.PERIODIC\`
指定时器以通道的配置频率定期运行。
::::
