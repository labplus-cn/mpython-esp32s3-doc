...
.. method:: poll.register(obj[, eventmask])
   ...
   - ``obj`` :被监控的对象
     ...
     注意：像标志 ``select.POLLHUP`` 和 ``select.POLLERR`` 不是有效的输入 `eventmask`（这些是从poll()返回的不请自来的事件，无论他们是否被要求）。
     这个语义是按POSIX。
...
.. method:: poll.modify(obj, eventmask)
   ...
   修改已注册的对象 ``obj`` 的eventmask。如果未注册obj，OSError 则会出现ENOENT错误。
...
.. method:: poll.poll(timeout=-1, /)
   ...
   event元素指定流发生的事件，并且是 ``select.POLL*`` 上述常量的组合。需要注意的是标志 ``select.POLLHUP`` 和 ``select.POLLERR`` 可以在任何时候（即使不要求）返回的，必须采取相应的行动（从调查未注册并有可能关闭相应的流），否则所有的进一步调用 ``poll()`` 可以用这些标志设置立即返回再次为这个流。
...
.. method:: poll.ipoll(timeout=-1, flags=0, /)
   ...
   如果flags为1，则采用事件的一次性行为：发生事件的流将自动重置其事件掩码（相当于poll.modify(obj, 0)），因此在使用poll.modify()设置新掩码之前，不会处理此类流的新事件。此行为对异步I/O调度程序很有用。
