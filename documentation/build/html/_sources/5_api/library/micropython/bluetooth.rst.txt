...
.. method:: BLE.irq(handler, trigger=0xffff)
    ...
    注:  ``addr``, ``adv_data`` 和 ``uuid``  元组中的项是引用的数据管理 :mod:`bluetooth` 模块(即相同的实例将被重新使用多次调用到事件处理程序)。
...
.. method:: BLE.gatts_register_services(services_definition)
    ...
    *services_definition* 是一个服务的列表，其中每个服务都是一个包含UUID和特征列表的二元元组。
    ...
    flags是一个按位或组合的 :data:`bluetooth.FLAG_READ`，:data:`bluetooth.FLAG_WRITE` 和 :data:`bluetooth.FLAG_NOTIFY` 。如下文所定义的值:
