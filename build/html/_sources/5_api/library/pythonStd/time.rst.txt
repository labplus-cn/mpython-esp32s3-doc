...
**Time Epoch**: Unix、Windows、WebAssembly、Alif、MIMXRT和RP2端口使用1970-01-01 00:00:00 UTC的POSIX系统时间的标准。其他嵌入式端口使用 ``2000-01-01 00:00:00 UTC`` 的时间。

纪元年份可以通过 ``gmtime(0)[0]`` 确定。

**维护实际日历日期/时间**: 这需要实时时钟（RTC）。在具备底层OS的系统中，RTC可能是隐式的。设置和维护实际日历时间应是OS/RTOS的职能，
...
.. function:: localtime([secs])
   ...
   ``gmtime()`` 函数返回UTC的日期时间元组， ``localtime()`` 返回本地时间的日期时间元组。
...
.. function:: mktime(date_time_tuple)
   ...
   此为 ``localtime()`` 的逆函数，其参数为一个表示本地时间的8元组。返回一个表示时间纪元以来的秒钟的整数。
...
.. function:: sleep(seconds)
   ...
   休眠给定秒数的时间。某些板可能接受浮点数作为秒数来休眠小数秒。注意：其他端口可能不接受浮点参数，为满足兼容性，使用 ``sleep_ms()`` 和 ``sleep_us()`` 函数。
...
.. function:: time()
   ...
   您不应依赖该函数来提供高于第二精度的结果。若您需要更高精度的绝对时间戳，请使用 ``time_ns()`` 函数。
   若相对时间可接受，则使用 ``ticks_ms()`` 和 ``ticks_us()`` 函数。若您需要日历时间，无参数的 ``gmtime()`` 或 ``localtime()`` 不失为佳选。
...
.. function:: time_ns()
   ...
   类似于 `time()` ，但返回自纪元以来的纳秒数，作为整数（通常是大整数，因此会在堆上分配）。
